...
### 🔔 即時通知與本機提醒
- **雙端通知支援**：網頁版支援標準瀏覽器推播通知；原生 App 版則支援 iOS 本地通知。
- **三重精準提醒**：包含「戰鬥結束提醒（重生前 5 分鐘）」、「重生前 1 分鐘卡位提醒」以及「重生完成提醒」，確保您與隊友不再錯過任何蘑菇！

### ⏱️ 智能最近房間與分頁記憶 (Room & Tab Recovery)
- **自動記憶停留房間與分頁**：即使將瀏覽器關閉或將 App 滑掉，下次重新打開時也會**自動連線並回到您上次停留的房間與區域分頁**，不需重新手動選擇或輸入！
- **最近房間歷史紀錄**：首頁會自動記憶您最近成功建立或加入的最多 5 個房間。再次打開網頁時，首頁會浮現玻璃質感的「⏱️ 最近加入的房間」列表，**一鍵點選即可直接連線進入**，免去反覆複製貼上代碼。
...
<details>
<summary>📱 原生 iOS App 打包與編譯 (Capacitor)</summary>

本專案已完整整合 **CapacitorJS**，支援直接打包編譯為原生 iOS App，並享有系統級原生本地通知功能（包括戰鬥結束、重生前 1 分鐘、重生完成之三重定時推播）。
...
2. **同步資源至 iOS 原生專案**：
   ...
   npx cap sync ios
   ```
3. **使用 Xcode 開啟專案**（需在 Mac 電腦並安裝 Xcode）：
   ```bash
   npx cap open ios
   ```
4. **於 Xcode 中執行**：
   - 於最左側點擊 `App`，在 `Signing & Capabilities` 頁籤中勾選 `Automatically manage signing` 並選取您的 Apple ID（Team）。
   - 頂部裝置選取您的 iPhone 實體手機，點選左上角 **▶️ (Run)** 即可編譯並安裝至手機！
</details>
